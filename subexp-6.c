...
父进程创建管道，得到两个文件描述符指向管道的两端；父进程使用 fork
创建子进程，子进程也有两个文件描述符指向同一个管道。此时，可以是父进程往管道里写，子进程从管道里面读；也可以是子进程往管道里写，父进程从管道里面读。这两条通路都是可选的，但是不能都选。

思考 2：
通过调整 sleep() 函数的位置，可以控制子进程 1、2 发送信息的顺序。这是通过 sleep()
函数挂起某一进程，延缓进程执行写入管道实现的。如果
sleep()使用不当，会使进程发送、接受信息混乱，甚至由于挂起错过读取管道内信息。

思考 3：
加锁、解锁确保了同一时刻只有一个进程能使用管道的写或堵操作，如果不使用 lockf()
函数，可能引发管道使用的冲突。
*/